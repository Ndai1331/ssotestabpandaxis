...
- Do not let any conversation exceed **90% context usage** or **200k tokens**; whichever threshold comes first is the hard stop.
- When either threshold is approaching, warn the user and recommend handoff before continuing substantial work.
- If work is still active at the threshold, create a concise handoff summary with current goal, completed work, pending tasks, changed files, commands run, test status, blockers, and next steps.
- After handoff, continue in a new conversation/thread when available instead of extending the overloaded conversation.
