...
public abstract class DocumentServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(hanhchinhsoDocumentServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(hanhchinhsoDocumentServiceModule).Assembly.FullName} assembly!");
        }
