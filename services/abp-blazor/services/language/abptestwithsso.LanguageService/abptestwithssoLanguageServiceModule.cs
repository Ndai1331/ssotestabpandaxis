...
using abptestwithsso.LanguageService.Data;
using Prometheus;
...
using Volo.Abp.LanguageManagement.EntityFrameworkCore;
using abptestwithsso.LanguageService.HealthChecks;
...
public class abptestwithssoLanguageServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<LanguageServiceRuntimeDatabaseMigrator>()
    ...
    private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<abptestwithssoLanguageServiceContractsModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}abptestwithsso.LanguageService.Contracts",
        ...
        Configure<AbpVirtualFileSystemOptions>(options =>
        {
            options.FileSets.AddEmbedded<abptestwithssoLanguageServiceModule>();
        });
    }

    private void ConfigureHealthChecks(ServiceConfigurationContext context)
    {
        context.Services.AddLanguageServiceHealthChecks();
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"LanguageService", "LanguageService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo {Title = "LanguageService API", Version = "v1"});
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddAbpDbContext<LanguageServiceDbContext>(options =>
        {
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<LanguageServiceDbContext>(c =>
            {
                c.UseNpgsql(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<LanguageServiceDbContext>();
            });
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(abptestwithssoLanguageServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = "LanguageService";
                    opts.RootPath = "language-management";
                });
        });
    }
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "LanguageService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("LanguageService");
    }
