...
public abstract class LanguageServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(hanhchinhsoLanguageServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(hanhchinhsoLanguageServiceModule).Assembly.FullName} assembly!");
        }
