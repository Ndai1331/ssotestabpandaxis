...
public abstract class IdentityServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(hanhchinhsoIdentityServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(hanhchinhsoIdentityServiceModule).Assembly.FullName} assembly!");
        }
