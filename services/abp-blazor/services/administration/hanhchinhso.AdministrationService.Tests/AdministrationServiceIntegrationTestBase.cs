...
public abstract class AdministrationServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(hanhchinhsoAdministrationServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(hanhchinhsoAdministrationServiceModule).Assembly.FullName} assembly!");
        }
