...
public abstract class AuditLoggingServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(abptestwithssoAuditLoggingServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(abptestwithssoAuditLoggingServiceModule).Assembly.FullName} assembly!");
        }
