...
using abptestwithsso.AuditLoggingService.HealthChecks;
using Prometheus;
...
public class abptestwithssoAuditLoggingServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<AuditLoggingServiceRuntimeDatabaseMigrator>()
    ...
    {
        context.Services.AddAuditLoggingServiceHealthChecks();
    }
    ...
    private void ConfigureVirtualFileSystem()
    {
        ...
        {
            options.FileSets.AddEmbedded<abptestwithssoAuditLoggingServiceModule>();
        });
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"AuditLoggingService", "AuditLoggingService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo {Title = "AuditLoggingService API", Version = "v1"});
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        Configure<AbpDbConnectionOptions>(options =>
        {
            ...
            options.Databases.Configure(AuditLoggingServiceDbContext.DatabaseName, database =>
            {
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<AuditLoggingServiceDbContext>(c =>
            {
                c.UseNpgsql(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<AuditLoggingServiceDbContext>();
            });
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(abptestwithssoAuditLoggingServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = "AuditLoggingService";
                    opts.RootPath = "audit-logging";
                });
        });
    }
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "AuditLoggingService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("AuditLoggingService");
    }
