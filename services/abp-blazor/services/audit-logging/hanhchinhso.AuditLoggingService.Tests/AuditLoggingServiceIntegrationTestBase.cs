...
public abstract class AuditLoggingServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(hanhchinhsoAuditLoggingServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(hanhchinhsoAuditLoggingServiceModule).Assembly.FullName} assembly!");
        }
