...
public class hanhchinhsoOrganizationServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<OrganizationServiceRuntimeDatabaseMigrator>()
    ...
    private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<hanhchinhsoOrganizationServiceContractsModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}hanhchinhso.OrganizationService.Contracts",
        ...
        Configure<AbpVirtualFileSystemOptions>(options =>
        {
            options.FileSets.AddEmbedded<hanhchinhsoOrganizationServiceModule>();
        });
    }

    private void ConfigureHealthChecks(ServiceConfigurationContext context)
    {
        context.Services.AddOrganizationServiceHealthChecks();
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"OrganizationService", "OrganizationService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo {Title = "OrganizationService API", Version = "v1"});
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<OrganizationServiceDbContext>(c =>
            {
                c.UseNpgsql(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<OrganizationServiceDbContext>();
            });
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(hanhchinhsoOrganizationServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = "OrganizationService";
                    opts.RootPath = "organization-management";
                });
        });
    }
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "OrganizationService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("OrganizationService");
    }
