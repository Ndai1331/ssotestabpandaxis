...
using abptestwithsso.AIManagementService.Data;
using Prometheus;
...
using abptestwithsso.AIManagementService.HealthChecks;
...
public class abptestwithssoAIManagementServiceModule : AbpModule
{
    ...
    public override async Task OnPreApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await scope.ServiceProvider
            .GetRequiredService<AIManagementServiceRuntimeDatabaseMigrator>()
    ...
    {
        context.Services.AddAIManagementServiceHealthChecks();
    }
    ...
    private void ConfigureVirtualFileSystem()
    {
        ...
        {
            options.FileSets.AddEmbedded<abptestwithssoAIManagementServiceModule>();
        });
    }
    ...
    private void ConfigureSwagger(ServiceConfigurationContext context, IConfiguration configuration)
    {
        if (IsSwaggerEnabled(configuration))
        {
            context.Services.AddAbpSwaggerGenWithOAuth(
                ...
                {
                    {"AIManagementService", "AIManagementService Service API"}
                },
                options =>
                {
                    options.SwaggerDoc("v1", new OpenApiInfo {Title = "AIManagementService API", Version = "v1"});
    ...
    private void ConfigureDatabase(ServiceConfigurationContext context)
    {
        ...
        {
            options.Databases.Configure("AIManagementService", database =>
            {
                database.MappedConnections.Add(AIManagementDbProperties.ConnectionStringName);
            });
        ...
        context.Services.AddAbpDbContext<AIManagementServiceDbContext>(options =>
        {
        ...
        Configure<AbpDbContextOptions>(options =>
        {
            ...
            options.Configure<AIManagementServiceDbContext>(c =>
            {
                c.UseNpgsql(b =>
                {
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<AIManagementServiceDbContext>();
            });
    ...
    private void ConfigureAutoControllers()
    {
        Configure<AbpAspNetCoreMvcOptions>(options =>
        {
            options
                ...
                .Create(typeof(abptestwithssoAIManagementServiceModule).Assembly, opts =>
                {
                    opts.RemoteServiceName = "AIManagementService";
    ...
    {
        options.SwaggerEndpoint("/swagger/v1/swagger.json", "AIManagementService API");
        options.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
        options.OAuthScopes("AIManagementService");
    }
