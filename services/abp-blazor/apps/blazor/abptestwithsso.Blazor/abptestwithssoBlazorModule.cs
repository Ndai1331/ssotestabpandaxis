...
using abptestwithsso.Blazor.Client;
using abptestwithsso.Blazor.Client.Navigation;
using abptestwithsso.Blazor.Components;
...
public class abptestwithssoBlazorModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(abptestwithssoBlazorModule).Assembly
            );
        });
    ...
    private void ConfigureLocalization(IWebHostEnvironment hostingEnvironment)
    {
        ...
        {
            options.FileSets.AddEmbedded<abptestwithssoBlazorModule>();
        });
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<abptestwithssoBlazorModule>(hostingEnvironment.ContentRootPath);
            });
        }
    ...
    private void ConfigureBundles()
    {
        ...
        Configure<AbpBundlingOptions>(options =>
        {
            ...
            globalStyles.AddContributors(typeof(abptestwithssoStyleBundleContributor));

            var globalScripts = options.ScriptBundles.Get(BlazorWebAssemblyMudBlazorStandardBundles.Scripts.Global);
            globalScripts.AddContributors(typeof(abptestwithssoScriptBundleContributor));
    ...
    private void ConfigureSwaggerServices(IServiceCollection services)
    {
        services.AddAbpSwaggerGen(
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "abptestwithsso API", Version = "v1" });
    ...
    private void ConfigureCache(IConfiguration configuration)
    {
        ...
        {
            options.KeyPrefix = "abptestwithsso:";
        });
    }
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("abptestwithsso");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]!);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "abptestwithsso-Protection-Keys");
        }
    }
    ...
    private void ConfigureMenu(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new abptestwithssoMenuContributor(configuration));
        });
    }
    ...
    private void ConfigureRouter()
    {
        ...
        {
            options.AppAssembly = typeof(abptestwithssoBlazorModule).Assembly;
            options.AdditionalAssemblies.Add(typeof(abptestwithssoBlazorClientModule).Assembly);
        });
    }
