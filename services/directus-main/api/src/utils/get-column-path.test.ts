...
/*
{
  ...
  collection: 'articles',
  aliasMap: {
    author: { alias: 'grenv', collection: 'axis_users' },
	'author.role': { alias: 'ljnsv', collection: 'axis_roles' },
  },
  relations: []
...
test('Returns correct alias for m2o', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		collection: 'articles',
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'axis_users' },
			'author.role': { alias: 'grenv', collection: 'axis_roles' },
		},
		relations: [
			{
				collection: 'articles',
	...
	expect(result.targetCollection).toBe('axis_roles');
});
...
test('Returns correct alias when there are multiple joins to the same table', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'axis_users' },
			'author.role': { alias: 'grenv', collection: 'axis_roles' },
			'author.secondary_role': { alias: 'psgwn', collection: 'axis_roles' },
		},
		relations: [
			{
				collection: 'articles',
