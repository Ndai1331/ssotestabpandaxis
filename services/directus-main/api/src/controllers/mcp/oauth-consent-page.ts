/**
 * Server-rendered OAuth consent page using Liquid templates and Axis theme tokens.
 *
...
// ---------------------------------------------------------------------------
// Axis logo (inlined to avoid runtime filesystem dependency on app source)
// ---------------------------------------------------------------------------
 ...
 * Inlines Axis theme tokens as CSS custom properties (same logic as @directus/themes
...
/** Theme and branding options from axis_settings + user appearance preference. */
 ...
 * - `logoUrl` / `logoDataUri`: custom project logo URL or default Axis logo as data URI
...
function buildTemplateData(opts: PageOpts) {
	...
	return {
		...
		logoAlt: customLogo ? opts.projectName : 'Axis',
	};
}
