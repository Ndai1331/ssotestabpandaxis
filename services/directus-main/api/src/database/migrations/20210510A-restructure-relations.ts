...
export async function up(knex: Knex): Promise<void> {
	...
	await knex('axis_relations').update({ one_deselect_action: 'nullify' });

	await helper.schema.changeToType('axis_relations', 'sort_field', 'string', {
		length: helper.schema.getColumnNameMaxLength(),
	});

	await helper.schema.changeToType('axis_relations', 'one_deselect_action', 'string', {
		nullable: false,
		default: 'nullify',
	});
}
