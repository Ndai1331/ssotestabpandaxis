...
export async function up(knex: Knex): Promise<void> {
	...
	await knex('axis_users').update({ appearance: 'dark' }).where({ theme: 'dark' });
	await knex('axis_users').update({ appearance: 'light' }).where({ theme: 'light' });
	...
	await knex('axis_settings').update({ project_color: '#6644ff' }).whereNull('project_color');

	await knex.schema.alterTable('axis_settings', (table) => {
		table.string('project_color').defaultTo('#6644FF').notNullable().alter();
		table.uuid('public_favicon').references('axis_files.id');
