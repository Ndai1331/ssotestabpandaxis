...
export async function up(knex: Knex): Promise<void> {
	...
	await knex.schema.createTable('axis_shares', (table) => {
		...
		table.string('item');
		table.uuid('role').references('id').inTable('axis_roles').onDelete('CASCADE');
	...
	await knex.schema.alterTable('axis_sessions', (table) => {
		...
		table.uuid('share').references('id').inTable('axis_shares').onDelete('CASCADE');
	});
}
...
export async function down(knex: Knex): Promise<void> {
	...
	await knex.schema.dropTable('axis_shares');
}
