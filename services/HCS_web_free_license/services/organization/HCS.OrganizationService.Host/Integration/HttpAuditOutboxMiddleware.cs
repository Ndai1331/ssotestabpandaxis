...
{
    public async Task InvokeAsync(HttpContext context, OrganizationDbContext db)
    {
    ...
    private async Task InvokeReadOnlyAsync(HttpContext context, OrganizationDbContext db, ILogger logger)
    {
        var started = DateTime.UtcNow;
        var timer = Stopwatch.StartNew();
        Exception? failure = null;
        ...
        if (failure is not null)
        {
            logger.LogError(failure, "Organization request failed for {Method} {Path} ({CorrelationId})",
        ...
        {
            db.ChangeTracker.Clear();
            logger.LogCritical(auditError, "Failed to persist Organization read audit outbox record for {CorrelationId}", context.TraceIdentifier);
        }
    ...
    private static async Task PersistFailedAuditAsync(HttpContext context, OrganizationDbContext db,
        ILogger logger, DateTime started, Stopwatch timer, Exception failure)
    {
        ...
        {
            db.ChangeTracker.Clear();
            logger.LogCritical(auditError, "Failed to persist Organization failure audit outbox record for {CorrelationId}", context.TraceIdentifier);
        }
    }

    private static async Task PersistAuditAsync(HttpContext context, OrganizationDbContext db,
        DateTime started, Stopwatch timer, Exception? failure)
    {
        ...
        var audit = new AuditRecordCapturedEto(Guid.NewGuid(), "HCS.OrganizationService", "HCS.OrganizationService",
        ...
        db.OutboxMessages.Add(OrganizationOutbox.Audit(audit, context.TraceIdentifier, started));
