...
internal static class BffLogoutUrlBuilder
{
    public static string Build(IConfiguration configuration, string returnUrl)
    {
        ...
        {
            throw new InvalidOperationException("The BFF logout return URL must be an absolute HTTPS URL.");
        }

        return new Uri(origin, $"bff/logout?returnUrl={Uri.EscapeDataString(returnUrl)}").AbsoluteUri;
    }
}
