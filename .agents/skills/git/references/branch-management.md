...
### Task9 Agent Branches

For `task9-workspace` and all `services/*` submodules, this section overrides the generic convention below.

Create every feature/fix branch from latest `origin/main`, never from local `main`, detached HEAD, or an app-created worktree branch:

```bash
git fetch origin main --prune
git checkout -B <agent>_<type>/<slug> origin/main
```
...
If a Claude/Codex app auto-created a worktree on a stale or incorrectly named branch, immediately move the work to a valid branch created from `origin/main`.
