...
If the current repository is `task9-workspace` or any `services/*` submodule, do not use generic deploy detection. Follow the workspace deploy rules in `SKILLS.md` and `CLAUDE.md`.

Before any deploy to `test` or `main`:

```bash
CURRENT_BRANCH=$(git branch --show-current)
git fetch origin main test --prune
git checkout "$CURRENT_BRANCH"
git merge origin/main
```

Hard stop if:
- Branch is missing one of these prefixes: `claude_feat/`, `claude_fix/`, `codex_feat/`, `codex_fix/`.
- Latest `origin/main` has not been merged into the current branch.
- UI commit message for `test/main` does not start with `[WEB]`.
- API commit message for `test/main` does not start with `[API]`.
- GitHub Actions is not watched to completion after push.
